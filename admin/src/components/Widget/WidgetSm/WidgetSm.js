import { Visibility } from "@material-ui/icons";
import React, { useEffect, useState, useContext } from "react";
import classes from "./WidgetSm.module.css";
import HttpHook from "../../../hooks/HttpHook";
import { AuthContext } from "../../../store/auth/auth-context";

function WidgetSm() {
  const [newUsers, setNewUsers] = useState([]);
  const { sendRequest } = HttpHook();
  const DS = process.env.REACT_APP_DOMAIN_SERVER;
  const { token } = useContext(AuthContext);

  useEffect(() => {
    const getStats = async () => {
      const response = await sendRequest(`${DS}/user?new=true`, "GET", null, {
        Authorization: "Bearer " + token,
      });

      setNewUsers(response.users);
    };
    getStats();
  }, [sendRequest, DS, token]);
  return (
    <div className={classes.widgetSm}>
      <span className={classes.widgetSmTitle}>
        New member
        <ul className={classes.widgetSmList}>
          {newUsers.map((user) => {
            return (
              <li key={user._id} className={classes.widgetSmListItem}>
                <img
                  className={classes.widgetSmImg}
                  src={
                    user.profilePic ||
                    "data:image/png;base64,"
                  }
                  alt=""
                />
                <div className={classes.widgetSmUserInfo}>
                  <span className={classes.widgetSmUsername}>
                    {user.username}
                  </span>
                </div>
                <button className={classes.widgetSmButton}>
                  <Visibility className={classes.widgetSmIcon} /> Display
                </button>
              </li>
            );
          })}
        </ul>
      </span>
    </div>
  );
}

export default WidgetSm;
